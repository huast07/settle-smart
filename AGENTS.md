...
- **ADK 2.0 workflow style**: Use `google-adk>=2.0.0a0` and build orchestration with the graph `Workflow` API: function nodes, explicit edges, and `RequestInput` for human-in-the-loop steps. Do not use the ADK 1.x `SequentialAgent` or root-level `LlmAgent` orchestration style.
- **Local auth**: Keep local credentials in `app/.env`. Use either AI Studio API key mode or Google Cloud Vertex AI mode, not both.
